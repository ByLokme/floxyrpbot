...
client.on("message", async  msg => {
 var i = await db.fetch(`reklamengel_${msg.guild.id}`)
    if (i == 'acik') {
              ...
              if (küfür.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_WEBHOOKS")) {
                    ...
                    .setColor(0xffa300)
                    .setFooter('FloxyKüfür Sistemi', client.user.avatarURL)
                    .setAuthor(msg.guild.owner.user.username, msg.guild.owner.user.avatarURL)
                    .setDescription("Floxy, " + `**${msg.guild.name}**` + " adlı sunucunuzda küfür eden birini yakaladım.")
                    .setThumbnail('https://cdn.discordapp.com/avatars/714134230090907739/9f43b0ed434e06d7b51b68920073643b.png?size=2048')
                    .addField('** Küfür Eden Kişi**', '**Kullanıcı:** '+ msg.author.tag +'\n**ID:** '+ msg.author.id, true)
                    .addField('** Engellenen mesaj**', msg.content, true)
                    .setTimestamp()                   
                    msg.guild.owner.user.send(embed)                       
                    return msg.channel.send(`${msg.author}, Küfür Etmek Yasak! Senin Mesajını Özelden Kurucumuza Gönderdim. :rage:`).then(msg => msg.delete(25000));
                  }              
                } catch(err) {
                  console.log(err);
                }
...
if (!oldMsg.guild) return;
  ...
  if (i == 'aktif') {
      ...
      try {
          if (!oldMsg.member.hasPermission("MANAGE_WEBHOOKS")) {
              newMsg.delete();
              
        const embed1 = new Discord.RichEmbed()
        .setColor("BLUE")
        .setDescription(`${oldMsg.author} Ben Zeki Bir Botum Mesajını Editlersende Küfür Edemezsin**!** :rage:`)
...
client.on("guildMemberAdd", async member => {
  ...
  if(!sayı) return
  if(!kanal) return
  client.channels.get(kanal).send(`:loudspeaker: :inbox_tray: ${member} adlı üye sunucuya girdi **${sayı}** kişi olamamıza **${sayı - member.guild.members.size}** üye kaldı! Şuan **${member.guild.memberCount}** kişiyiz`)
})
client.on("guildMemberRemove", async member => {
  ...
  if(!sayı) return
  if(!kanal) return
  client.channels.get(kanal).send(`:loudspeaker: :outbox_tray: ${member} adlı üye sunucudan çıktı **${sayı}** kişi olamamıza **${sayı - member.guild.members.size}** üye kaldı! Şuan **${member.guild.memberCount}** kişiyiz`)
})
...
//caps engel XDDDDDDDDDDDDDDDDDDD
client.on("message", async msg => {
  if(msg.channel.type === "dm") return
  if(msg.author.bot) return;  
...
client.on("messageUpdate", async (oldMsg, newMsg, msg) => {
  ...
  if(newMsg.content == caps) {
    ...
    if(!yashinu && !newMsg.content.includes('@everyone') && !newMsg.content.includes('@here')) {
      newMsg.delete()
      return newMsg.channel.send(`${msg.author} Fazla büyük harf kullanmamalısın!`)
    }
  }
});